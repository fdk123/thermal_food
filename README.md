# Thermal Food

Thermal Food is a small Forge addon for the Thermal Series that adds a collection of food, drinks, cheeses, desserts, and meals built around Thermal Cultivation ingredients.

This branch targets Minecraft 1.20.1.

## Features

- Adds 32 new food and drink items.
- Includes cheeses, pies, ice cream variants, salads, rice dishes, pizza, pasta, tea, coffee drinks, cocoa, and more.
- Uses Thermal Series crops and ingredients in recipes, including rice, tea, coffee, tomato, onion, bell pepper, spinach, radish, sadiroot, strawberry, and peanut.
- New feature: Minecolonies, Farmers Delight (and Farmers Respite) and Immersive Cooking integration.
- Includes English and Russian localization.

## Requirements

- Minecraft 1.20.1
- Forge 47.x
- CoFH Core 11.x
- Thermal Foundation 11.x
- Thermal Cultivation 11.x

## All Thermal rights Reserved by Cult of the Full Hub / Team CoFH / CoFH
## Thermal Food team: code by fdk123, art (textures) by Stimortal